...
- **Fair-value coverage — DDM and EPV no longer vanish for trough-earnings payers** (`docs/valuation_fv_coverage_plan.md`, FV-1 … FV-8 — the full plan).
  ...
  - **FV-5:** new `fv_model_count` (how many sub-models fed the composite — Graham + PE count once when the EPS was reconstructed from `trailingPE`) and `fv_basis_thin` (`compute_scores`: has a fair value but `fv_model_count < 2`) surface a real-but-weakly-corroborated composite, distinct from `data_thin` (no composite). A `fv_basis_thin` row whose only anchor is a reconstructed EPS is now also treated as a degraded payload by the background fetcher and re-fetched on the 3 h short TTL instead of sitting for a full day (PAH3.DE in the reference portfolio). A genuinely data-poor but cleanly-fetched row (a real no-dividend / no-coverage small-cap) keeps its normal TTL — the heal is scoped to reconstructed-EPS payloads so it can't spin.
  - **FV-6:** the drawer / Analysis "Six-model fair value" ladder now explains itself. Each dark row shows a short reason in place of the empty bar ("no positive EPS", "net debt > earnings", "no enterprise value", "not a dividend payer", "no analyst coverage", "n/a for this sector", …) with the full phrase on hover (`components.six_model_ladder_reasons`); a **"basis · N of 6 models"** line sits under the composite, amber and flagged "lightly corroborated" when `fv_basis_thin`; and a single caption reconciles the FV-3 book/FCF substitution and the −10% analyst-target haircut so the printed rows match the composite (`components.six_model_ladder_caption`). `fair_value_ladder` gains `reasons` / `basis_count` / `basis_thin` params.
  - **FV-8:** the earnings-anchored models are **skipped for NAV-driven sectors** — Graham + EPV for `Real Estate` and `Financial Services`, plus P/E for `Real Estate` (`_GRAHAM_EPV_SKIP_SECTORS` / `_PE_SKIP_SECTORS`). A REIT books IFRS fair-value revaluation gains in EPS (so EPS and any P/E on it swing ±50% y/y); banks and insurers have no operating EBIT. These names now value off the FV-3 book-value model (a NAV proxy) + DDM + analyst — with the earnings trio skipped, `_trio` is 0 so the book-value fallback fires. `Utilities` are **not** affected (regulated, stable EPS, a real EBIT). Reference portfolio: AED.BR fair value 92 → 62 (was dragged up by a €131 Graham / €131 P/E on revaluation-inflated EPS); every held REIT now reads against ~1× NAV instead.
