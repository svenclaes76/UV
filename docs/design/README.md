...
| [`Uvalu Dividend Management.dc.html`](Uvalu%20Dividend%20Management.dc.html) | Dividend Management v2 — Portfolio → Dividend log (summary tiles, 13-column event log, annual income summary), Add/Edit dividend modal (declaration/ex/record/payment dates, gross/share, foreign WH, type, frequency, Gross → Foreign WH → BE 30% → Net preview), open-positions income/yield/YoC columns, Stock Detail "Dividend & income" panel, Dashboard upcoming-dividends card, dividend alert settings. From the separate `Uvalu Dividend Management-handoff.zip` Claude Design handoff (not the `edc1baa4-ffbe-46e8-828c-a545703d9112` project), built for `feature/dividend-management-v2`; requirements in the bundle's `uploads/Uvalu - Dividend Management Requirements.md`. **Deliberate deviations in the shipped app** (per user review, Sep 2026 — don't "fix" these back to the mockup): no "Import from market data" button (the import runs automatically once per session), the log download sits next to Add dividend as "Export", no declaration/record-date sub-lines or "· frequency" suffix in log rows, an unconfirmed (auto-imported) pay date is shown in amber with a tooltip instead of "· confirm", the annual summary card keeps the mockup's left-hand width but its button is labelled "Export", there is **no "Withholding by domicile" card**, and the Add/Edit dividend dialogs and CSV export have **no declaration date, record date or frequency** (dialogs ask only ex-date + payment date, then shares / per share / foreign WH, then type). |
| [`Uvalu Cash Management.dc.html`](Uvalu%20Cash%20Management.dc.html) | Cash Management v1 — Portfolio → cash strip (balance, invested-vs-cash bar, Deposit/Withdraw) and the full "Cash activity" page (5 tiles, filterable ledger with original amount / FX rate / EUR amount / running balance / Auto-Manual source, CSV export, Viewer read-only chip), the "Add cash transaction" modal (Deposit/Withdrawal/Fee/Interest/Adjustment, ECB-rate panel with manual/outage fallback, calculated-balance preview), Fees + "Cash after trade" on the Buy/Sell modals, the Dashboard "Cash" KPI tile and the Risk page's "invested portion only" banner. From the separate `Uvalu Cash Management.zip` Claude Design handoff (Sep 2026); implementation plan and decisions in [`docs/cash-management-implementation-plan.md`](../cash-management-implementation-plan.md). **Deliberate deviations in the shipped app** (per user decision, Sep 23 2026): Buy/Sell are **never blocked** by an insufficient balance — the shortfall is covered by an automatic, linked top-up deposit instead of the mockup's red "blocked" state; the `§` spec-note blocks are handoff annotations and are not rendered. |
